...
import { projectApi, financePaymentsApi } from "../../api";
...
function ActionButton({
  ...
  const variants = {
    approve: {
      ...
      color: "#a7e87a",
      hoverBg: "rgba(149,229,109,0.24)",
    },
    reject: {
      bg: "rgba(255,126,126,0.12)",
      border: "rgba(255,126,126,0.28)",
      color: "#fca5a5",
      hoverBg: "rgba(255,126,126,0.22)",
    },
    activate: {
      bg: "rgba(213,255,119,0.12)",
      border: "rgba(213,255,119,0.28)",
      color: "#c6f06a",
      hoverBg: "rgba(213,255,119,0.22)",
    },
    delete: {
      bg: "rgba(255,126,126,0.08)",
      border: "rgba(255,126,126,0.2)",
      color: "#fca5a5",
      hoverBg: "rgba(255,126,126,0.16)",
    },
    edit: {
...
export default function ProjectDetails() {
  ...
  const fetchProject = async () => {
    ...
    try {
      ...
      const [projectData, summaryRes] = await Promise.all([
        ...
        financePaymentsApi.getFundingSummary(id),
      ]);
  ...
  const handleAction = async (apiCall, actionName, actionKey) => {
    if (!window.confirm(`Are you sure you want to ${actionName} this project?`))
      return;
  ...
  const canActivate =
    ...
    (project.totalFunding || 0) >= (project.cost || 0);
        ...
        >
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "12px" }}>
            {project.status === "Pending" && (
              <>
                <ActionButton
                  ...
                  variant="approve"
                  onClick={() =>
                    handleAction(projectApi.approve, "approve", "approve")
                  }
                ...
                <ActionButton
                  ...
                  variant="reject"
                  onClick={() =>
                    handleAction(projectApi.reject, "reject", "reject")
                  }
            ...
            {project.status === "Approved" && (
              <ActionButton
                ...
                variant="activate"
                onClick={() =>
                  handleAction(projectApi.activate, "activate", "activate")
                }
          ...
          {project.status === "Approved" &&
            ...
            project.cost > 0 && (
              ...
              >
                ⚠️ Need additional{" "}
                {formatLKR((project.cost || 0) - (project.totalFunding || 0))}{" "}
                in funding before this project can be activated.
              </div>
            )}
        </div>
      </div>
